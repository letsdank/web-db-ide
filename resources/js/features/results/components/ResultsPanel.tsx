...
export function ResultsPanel({
    ...
    const pinnedVisibleColumnCount = visibleColumns.filter((column) => column.isPinned).length;

    const contextCell = cellMenuState.payload;
    const contextHeader = headerMenuState.payload;
    const isContextHeaderPinned = contextHeader
        ? visibleColumns.some(
            (column) => column.name === contextHeader.columnName && column.isPinned,
        )
        : false;

    function handleExportCsv() {
        downloadFile(
            buildExportFileName(activeTabTitle, 'csv'),
    ...
    function handleExportJson() {
        ...
        downloadFile(
            buildExportFileName(activeTabTitle, 'json'),
    ...
    function handleExportTsv() {
        downloadFile(
            buildExportFileName(activeTabTitle, 'tsv'),
    ...
    return (
        ...
        >
                        ...
                        ? [
                            {
                                ...
                                onClick: () => void copyText(contextCell.cell === null ? 'NULL' : String(contextCell.cell)),
                            },
                            {
                                key: 'copy-row-tsv',
                                text: t('workspace.copyRowAsTsv'),
                                onClick: () => void copyText(buildRowTsv(contextCell.row)),
                            },
                            {
                                key: 'copy-row-json',
                                text: t('workspace.copyRowAsJson'),
                                onClick: () => void copyText(buildRowJson(contextCell.row, visibleColumnNames)),
                            },
                            {
                                key: 'separator-1',
                            },
                            {
                                ...
                                onClick: () => void copyText(contextCell.columnName),
                            },
                        ]
                        : []
                }
                ...
                actions={
                    contextHeader
                        ? [
                            ...
                            {
                                key: 'pin',
                                text: isContextHeaderPinned
                                    ? t('workspace.unpinColumn')
                                    : t('workspace.pinColumn'),
                                onClick: () => {
                                    if (isContextHeaderPinned) {
                                        unpinColumn(contextHeader.columnName);
                                        return;
                                    }

                                    pinColumn(contextHeader.columnName);
                                },
                            },
                            {
                                key: 'hide-column',
                            ...
                            },
                            {
                                key: 'copy-column',
                                text: t('workspace.copyColumnName'),
                                onClick: () => void copyText(contextHeader.columnName),
                            },
                            {
                                key: 'separator-3',
                            },
                            {
                                key: 'reset-pins',
                                text: t('workspace.resetPinnedColumns'),
            ...
            <ResultsToolbar
                ...
                pinnedColumnCount={pinnedVisibleColumnCount}
